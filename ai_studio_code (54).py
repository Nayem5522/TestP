...
# [MODIFIED] Added new CSS for header, hero section, buttons, drawer menu, and footer.
index_html = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* [MODIFIED] Button and Link Styles */
  ...
  .download-button { background-color: var(--netflix-red); } /* Red color for download buttons */
  .download-button:hover { background-color: #f61f29; } /* Darker red on hover */
  ...
  @media (max-width: 768px) { 
    ...
    /* [NEW] Responsive fix for links container */
  ...
  <div class="detail-content-wrapper"><img class="detail-poster" src="{{ movie.poster or 'https://via.placeholder.com/400x600.png?text=No+Image' }}" alt="{{ movie.title }}">
    <div class="detail-info">
      ...
      {% elif movie.type == 'movie' %}
        <!-- [MODIFIED] Link Sections Wrapper -->
        <div class="links-wrapper">
            <div class="links-container">
                <!-- [MODIFIED] Streaming Links Column -->
    ...
    <form method="post" action="{{ url_for('admin') }}">
        ...
        <div id="movie_fields">
            <fieldset><legend>Movie Links</legend>
                ...
                <!-- [MODIFIED] Streaming Links - Now Dynamic -->
                <p><b>Streaming Links (Optional)</b></p>
                <div id="streaming_links_container"></div>
                <button type="button" onclick="addStreamingLinkField()" class="btn btn-secondary"><i class="fas fa-plus"></i> Add Streaming Link</button><hr>
...
<script>
    ...
    // [NEW] Function to add streaming link fields
    function addStreamingLinkField() {
        const container = document.getElementById('streaming_links_container');
        const newItem = document.createElement('div');
        newItem.className = 'dynamic-item';
        newItem.innerHTML = \`
            <button type="button" onclick="this.parentElement.remove()" class="btn btn-danger"><i class="fas fa-times"></i></button>
            <div class="form-group">
                <label>Streaming Quality (e.g., 480p):</label>
                <input type="text" name="streaming_quality[]" required />
            </div>
            <div class="form-group">
                <label>Streaming URL:</label>
                <input type="url" name="streaming_url[]" required />
            </div>
        \`;
  ...
  <form method="post">
    ...
    <div id="movie_fields">
        <fieldset><legend>Movie Links</legend>
            ...
            <!-- [MODIFIED] Streaming Links - Now Dynamic -->
            <p><b>Streaming Links (Optional)</b></p>
            <div id="streaming_links_container">
                {% if movie.streaming_links %}{% for link in movie.streaming_links %}
                <div class="dynamic-item">
                    <button type="button" onclick="this.parentElement.remove()" class="btn btn-danger"><i class="fas fa-times"></i></button>
                    <div class="form-group">
                        <label>Streaming Quality (e.g., 480p):</label>
                        <input type="text" name="streaming_quality[]" value="{{ link.quality }}" required />
                    </div>
                    <div class="form-group">
                        <label>Streaming URL:</label>
                        <input type="url" name="streaming_url[]" value="{{ link.url }}" required />
                    </div>
                </div>
                {% endfor %}{% endif %}
            </div>
...
def movie_detail(movie_id):
    try:
        ...
        # [NEW] Process trailer link for embedding
...
# [NEW] Generic route for categories
...
def admin():
    if request.method == "POST":
        # [MODIFIED] Admin POST logic to handle new fields
        movie_data = {
        ...
        # Add TMDB details if not already present from auto-fetch
        if not movie_data.get('tmdb_id'):
            tmdb_details = get_tmdb_details_from_title(movie_data['title'], movie_data['type'])
            if tmdb_details:
                # Prioritize form data over TMDB data for user-editable fields
        ...
        if movie_data['type'] == "movie":
            ...
            # [MODIFIED] Handle Dynamic Streaming Links
            streaming_qualities = request.form.getlist("streaming_quality[]")
            streaming_urls = request.form.getlist("streaming_url[]")
            movie_data['streaming_links'] = [
                {"quality": q.strip(), "url": u.strip()} 
                for q, u in zip(streaming_qualities, streaming_urls) if q.strip() and u.strip()
            ]
...
# [NEW] API route for TMDB fetcher
...
def telegram_webhook():
    ...
    if 'channel_post' in data:
        ...
        def get_or_create_content_entry(tmdb_details, parsed_details):
            # [MODIFIED] Added categories field to new entries
