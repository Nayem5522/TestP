...
# Define the categories available in the admin panel
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .top-category-nav { padding: 80px 0 20px 0; text-align: center; }
  ...
  @media (max-width: 768px) {
      ...
      .nav-item { display:flex; flex-direction:column; align-items:center; color: var(--text-dark); font-size:10px; }
  }
</style>
...
<div style="padding: 0 50px 50px; max-width: 1200px; margin: auto;">
    ...
    <div class="genres-section" style="margin-top:20px;">
        ...
        {% for genre in movie.genres %}<a href="{{ url_for('genres_page', genre_name=genre) }}" class="download-button">{{ genre }}</a>{% endfor %}
    </div>
    {% endif %}
    ...
    <!-- Download & Episode Logic would go here -->
...
<style>
  ...
  h1, h2, h3 { color: var(--netflix-red); }
  ...
  .checkbox-group label { background: var(--light-gray); padding: 10px; border-radius: 4px; }
...
<h2>Add/Edit Content Manually</h2>
<form method="post">
    ...
    <div class="form-group"><label>Poster Badge:</label><input type="text" name="poster_badge" id="poster_badge" placeholder="e.g., WEB-DL"></div>
    ...
    <!-- Add other custom fields like watch/download links here -->
...
def home():
    ...
    home_content = {}
    # Define which categories to show on the homepage and in what order
...
@app.route('/genres')
def genres_page():
    # This page now can list all unique genres from the database
...
@requires_auth
def admin_page():
    return render_template_string(admin_html, site_categories=SITE_CATEGORIES)
...
def save_content():
    ...
    movie_doc = {
        ...
        # You need to add inputs for these in your admin_html to save them
    ...
    return redirect(url_for('admin_page')) # Redirect to a new manage page in a real app
...
# Other routes like webhook, contact etc would be here
