    ...
    <!-- Download & Episode Logic from original file -->
      ...
      {% elif movie.type == 'movie' %}
        <div class="download-section">
          ...
          {% if movie.files %}<h3 class="section-title">Get from Telegram</h3>{% for file in movie.files | sort(attribute='quality') %}<a href="https://t.me/{{ bot_username }}?start={{ movie._id }}_{{ file.quality }}" class="action-btn" style="background-color: #2AABEE;"><i class="fa-brands fa-telegram"></i> Get {{ file.quality }}</a>{% endfor %}{% endif %}
        </div>
...
<form action="{{ url_for('save_content') }}" method="post">
    ...
    <fieldset>
        ...
        <!-- Add fields for download links and telegram files as needed -->
    </fieldset>
...
def manage_content():
    ...
    return render_template_string("""
        ...
        <style>body{font-family:sans-serif; background:#141414; color:#fff;} table{width:100%; border-collapse:collapse;} th,td{border:1px solid #333; padding:8px;} a{color:#e50914;}</style>
        </head><body>
        <h1>Manage Content</h1>
        <a href="{{ url_for('admin') }}">Add New Content</a>
...
# This is your original webhook function, untouched.
