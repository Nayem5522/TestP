...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .logo { font-family: 'Bebas Neue', sans-serif; font-size: 32px; color: var(--netflix-red); font-weight: 700; letter-spacing: 1px; display: flex; align-items: center; gap: 10px; }
  ...
  @media (max-width: 768px) {
      ...
      /* [MODIFIED] Mobile Hero Section Height */
...
<body>
<header class="main-nav"><a href="{{ url_for('home') }}" class="logo"><i class="fas fa-tv"></i> PmwBD</a><form method="GET" action="/" class="search-form"><input type="search" name="q" class="search-input" placeholder="Search..." value="{{ query|default('') }}" /></form></header>
<main>
  ...
  {% else %}
    <!-- [REMOVED] The tags/badges section is removed from here -->
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* [NEW] Dynamic button with wipe animation */
  ...
  .download-btn-dynamic .btn-text {
      position: relative;
      z-index: 2;
  }
  .download-btn-dynamic::before {
      content: '';
      position: absolute;
      top: 0;
      right: 0;
  ...
  .download-btn-dynamic:hover::before {
  ...
  .action-btn, .download-button, .download-btn-dynamic { display: block; width: 100%; max-width: 320px; margin: 0 auto 10px auto; }
    ...
    <div class="detail-info">
      ...
      {% elif movie.type == 'movie' %}
        <div class="download-section">
          <!-- [MODIFIED] Dynamic Download Buttons -->
          {% if movie.download_links %}
            <h3 class="section-title">Download Links</h3>
            {% for link in movie.download_links %}
              <a class="download-btn-dynamic" href="{{ link.url }}" target="_blank" rel="noopener">
                  <i class="fas fa-download"></i> <span class="btn-text">{{ link.text }}</span>
              </a>
            {% endfor %}
          {% endif %}
...
genres_html = """
<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no" /><title>{{ title }} - PmwBD</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Roboto:wght@400;500;700&display=swap');
...
<head>
    ...
    <style>
        ...
        .dynamic-item .input-group { display: flex; gap: 10px; }
...
<div class="admin-container">
    ...
    <form method="post" action="{{ url_for('admin') }}">
        ...
        <div id="movie_fields">
            <fieldset><legend>Movie Links</legend>
                <div class="form-group"><label>Watch Link (Embed URL):</label><input type="url" name="watch_link" placeholder="https://..."/></div><hr>
                <p><b>Custom Download Buttons</b></p>
                <div id="download_buttons_container"></div>
                <button type="button" onclick="addDownloadButtonField()" class="btn btn-secondary"><i class="fas fa-plus"></i> Add Download Button</button>
                <hr>
    ...
    {% else %}
    <tr><td colspan="3" style="text-align: center; padding: 20px;">No content found.</td></tr>
    {% endfor %}
    </tbody></table>
    </div>
...
<script>
    ...
    // [NEW] Function to add download button fields
    function addDownloadButtonField() {
        const c = document.getElementById('download_buttons_container');
        ...
        d.innerHTML = \`
            ...
            <div class="input-group">
                <div style="flex: 1;"><label>Button Text:</label><input type="text" name="button_texts[]" placeholder="e.g., Download 720p" required /></div>
                <div style="flex: 2;"><label>Button URL:</label><input type="url" name="button_urls[]" placeholder="https://..." required /></div>
            </div>
        \`;
        c.appendChild(d);
    }
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Edit Content - PmwBD</title>
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Roboto:wght@400;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css">
    <style>
        :root { --netflix-red: #E50914; --netflix-red-dark: #B20710; --netflix-black: #141414; --dark-gray: #222; --light-gray: #333; --text-light: #f5f5f5; --text-medium: #999; }
...
<div class="admin-container">
  ...
  <form method="post">
    ...
    <div id="movie_fields">
        <fieldset><legend>Movie Links</legend>
            <div class="form-group"><label>Watch Link:</label><input type="url" name="watch_link" value="{{ movie.watch_link or '' }}" /></div><hr><p><b>Custom Download Buttons</b></p>
            <div id="download_buttons_container">
                {% if movie.type == 'movie' and movie.download_links %}{% for link in movie.download_links %}
                <div class="dynamic-item">
                    <button type="button" onclick="this.parentElement.remove()" class="btn btn-danger"><i class="fas fa-times"></i></button>
                    <div class="input-group">
                        <div style="flex: 1;"><label>Button Text:</label><input type="text" name="button_texts[]" value="{{ link.text }}" placeholder="e.g., Download 720p" required /></div>
                        <div style="flex: 2;"><label>Button URL:</label><input type="url" name="button_urls[]" value="{{ link.url }}" placeholder="https://..." required /></div>
                    </div>
                </div>
                {% endfor %}{% endif %}
            </div>
            <button type="button" onclick="addDownloadButtonField()" class="btn btn-secondary"><i class="fas fa-plus"></i> Add Download Button</button>
            <hr><p><b>Get from Telegram</b></p>
    ...
    <fieldset><legend>Options</legend>
        <div class="form-group"><input type="checkbox" name="is_trending" id="is_trending" value="true" {% if movie.is_trending %}checked{% endif %}><label for="is_trending" style="display: inline-block;">Is Trending?</label></div>
        <div class="form-group"><input type="checkbox" name="is_coming_soon" id="is_coming_soon" value="true" {% if movie.is_coming_soon %}checked{% endif %}><label for="is_coming_soon" style="display: inline-block;">Is Coming Soon?</label></div>
    </fieldset>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Contact Us / Report - PmwBD</title>
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Roboto:wght@400;700&display=swap" rel="stylesheet">
    <style>
...
def admin():
    if request.method == "POST":
        ...
        movie_data = {
            ...
            "type": content_type,
            "poster_badge": poster_badge,
            "poster": PLACEHOLDER_POSTER,
            "overview": "",
            "is_trending": False,
            "is_coming_soon": False,
            "download_links": [], "files": [], "episodes": [], "season_packs": [], 
            "languages": [], "genres": [], "tmdb_id": None
        }
        ...
        if content_type == "movie":
            ...
            # [MODIFIED] Process dynamic download buttons
            button_texts = request.form.getlist('button_texts[]')
            button_urls = request.form.getlist('button_urls[]')
            movie_data["download_links"] = [{"text": text, "url": url} for text, url in zip(button_texts, button_urls) if text.strip() and url.strip()]
            ...
            for q, mid in zip(tg_qualities, tg_message_ids):
                if q.strip() and mid.strip():
                    try:
                        movie_data['files'].append({"quality": q.strip(), "message_id": int(mid)})
                    except (ValueError, TypeError):
                        print(f"WARN: Invalid message_id '{mid}' for quality '{q}'. Skipping.")
            ...
            for s, e, t, m, w in zip(seasons, ep_nums, ep_titles, ep_msg_ids, ep_watch_links):
                if s.strip() and e.strip() and (m.strip() or w.strip()):
                    try:
                        episode = {
                            "season": int(s), "episode_number": int(e),
                            "title": t.strip(), "watch_link": w.strip() or None,
                            "message_id": int(m) if m.strip() else None
                        }
                        movie_data['episodes'].append(episode)
                    except (ValueError, TypeError):
                        print(f"WARN: Invalid episode data for S{s}E{e}. Skipping.")

        movies.insert_one(movie_data)
        tmdb_details = get_tmdb_details_from_api(movie_data['title'], content_type)
        if tmdb_details:
            movies.update_one({"_id": movie_data['_id']}, {"$set": tmdb_details})
...
def edit_movie(movie_id):
    ...
    if request.method == "POST":
        ...
        if not movie_obj.get('tmdb_id'):
            ...
            if tmdb_details: update_data.update(tmdb_details)
        ...
        if content_type == "movie":
            ...
            button_texts = request.form.getlist('button_texts[]')
            button_urls = request.form.getlist('button_urls[]')
            update_data["download_links"] = [{"text": text, "url": url} for text, url in zip(button_texts, button_urls) if text.strip() and url.strip()]
            ...
            movies.update_one({"_id": obj_id}, {"$set": update_data, "$unset": {"episodes": "", "season_packs": "", "links": ""}})
        else: # series
            ...
            movies.update_one({"_id": obj_id}, {"$set": update_data, "$unset": {"links": "", "watch_link": "", "files": "", "download_links": ""}})
                ...
                if not existing_entry:
                    base_doc = {
                        **tmdb_details, "type": "movie" if parsed_details['type'] == 'movie' else "series",
                        "languages": [], "episodes": [], "season_packs": [], "files": [], "download_links": [],
