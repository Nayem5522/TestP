...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .download-button, .episode-button { display: inline-flex; align-items: center; gap: 8px; padding: 10px 15px; color: white; text-decoration: none; border-radius: 4px; font-weight: 700; transition: background-color 0.3s ease; margin-bottom: 10px; text-align: center; }
  ...
  @media (max-width: 768px) { 
    ...
    .episode-item { flex-direction: column; align-items: stretch; gap: 10px; } .episode-buttons { justify-content: center !important; } .episode-button { flex-grow: 1; justify-content: center;}
    ...
    <div class="detail-info">
      ...
      {% elif movie.type == 'series' %}
        <div class="episode-section">
          ...
          <!-- [MODIFIED] DYNAMIC EPISODE BUTTONS -->
          {% if movie.episodes %}
            {% for ep in movie.episodes | sort(attribute='episode_number') | sort(attribute='season') %}
              <div class="episode-item">
                <div class="episode-info" style="flex-grow: 1;">
                  <span class="episode-title">Season {{ ep.season }} - Episode {{ ep.episode_number }}</span>
                  {% if ep.title %}<p style="font-size: 0.8rem; color: var(--text-dark); margin-top: 5px;">{{ ep.title }}</p>{% endif %}
                </div>
                <div class="episode-buttons" style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: flex-end; align-items: center;">
                  {% if ep.links %}
                    {% for link in ep.links %}
                      <a href="{{ link.url }}" target="_blank" class="episode-button" style="background-color: #007bff;"><i class="fas fa-play"></i> {{ link.text }}</a>
                    {% endfor %}
                  {% endif %}
                  {% if ep.message_id %}
...
<script>
    ...
    // [MODIFIED] AddEpisodeField for dynamic buttons
    function addEpisodeField() {
        ...
        d.innerHTML = `<button type="button" onclick="this.parentElement.remove()" class="btn btn-danger">Remove Episode</button>
                       ...
                       <hr>
                       <p><b>Custom Links:</b></p>
                       <div class="form-group"><label>Links (One per line: Button Text | URL):</label><textarea name="episode_links[]" rows="4" placeholder="e.g., Watch Online | https://...\\nDownload 720p | https://..."></textarea></div>
                       <div class="form-group"><label>Telegram Message ID (Optional):</label><input type="number" name="episode_message_id[]" /></div>`;
        c.appendChild(d);
    }
  ...
  <form method="post">
    ...
    <div id="episode_fields" style="display: none;">
      ...
      <fieldset><legend>Individual Episodes</legend>
        ...
        <!-- [MODIFIED] EPISODE LOOP FOR EDITING DYNAMIC BUTTONS -->
        {% if movie.type == 'series' and movie.episodes %}{% for ep in movie.episodes | sort(attribute='episode_number') | sort(attribute='season') %}<div class="dynamic-item">
            ...
            <hr><p><b>Custom Links:</b></p>
            <div class="form-group">
                <label>Links (One per line: Button Text | URL):</label>
                <textarea name="episode_links[]" rows="4" placeholder="e.g., Watch Online | https://...">{% for link in ep.links %}{{ link.text }} | {{ link.url }}{% if not loop.last %}
{% endif %}{% endfor %}</textarea>
            </div>
            <div class="form-group"><label>Telegram Message ID (Optional):</label><input type="number" name="episode_message_id[]" value="{{ ep.message_id or '' }}" /></div>
...
def admin():
    if request.method == "POST":
        ...
        if movie_data['type'] == "movie":
        ...
        else: # [MODIFIED] series logic for dynamic buttons
            ...
            for s, e, t, links_text, m in zip(seasons, ep_nums, ep_titles, ep_links_texts, ep_msg_ids):
