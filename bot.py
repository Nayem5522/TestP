...
.hero-title { font-family: 'Roboto', sans-serif; font-size: 3.0rem; text-transform: uppercase; font-weight: 700; margin-bottom: 1rem; line-height: 1.2; text-shadow: 2px 2px 5px rgba(0,0,0,0.7); }
...
<header class="main-nav">
    ...
    <div class="nav-left">
        ...
        <a href="{{ url_for('home') }}" class="logo">
            ...
            <img src="https://i.postimg.cc/bwMx2P6J/IMG-20250817-043812-084-removebg-preview.png" alt="PmwBD Logo" style="height: 60px;">
        </a>
    </div>
    <div class="nav-right">
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .download-button { display: block; width: 100%; padding: 12px 20px; color: white; text-decoration: none; border-radius: 4px; font-weight: 700; transition: background-color 0.3s ease; margin-bottom: 10px; text-align: center; }
  .download-button { background-color: var(--netflix-red); }
  .download-button:hover { background-color: #f61f29; }
  
  /* [MODIFIED] Episode styling */
  .episode-item { display: flex; flex-direction: column; align-items: stretch; margin-bottom: 15px; padding: 15px; border-radius: 5px; background-color: #1a1a1a; border-left: 4px solid var(--netflix-red); }
  .episode-info { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; }
  .episode-title { font-size: 1.1rem; font-weight: 500; color: #fff; }
  .episode-buttons { display: flex; flex-wrap: wrap; gap: 10px; }
  .episode-button { display: inline-flex; flex: 1; justify-content: center; align-items: center; gap: 8px; padding: 10px 15px; color: white; text-decoration: none; border-radius: 4px; font-weight: 700; transition: background-color 0.3s ease; text-align: center; }
  .episode-button.stream-btn { background-color: #007bff; } .episode-button.stream-btn:hover { background-color: #0069d9; }
  .episode-button.download-btn { background-color: var(--netflix-red); } .episode-button.download-btn:hover { background-color: #f61f29; }
  ...
  @media (max-width: 768px) { 
    ...
    .episode-info { flex-direction: column; align-items: center; gap: 5px; margin-bottom: 10px; }
    ...
    <div class="detail-info">
      ...
      {% elif movie.type == 'series' %}
        <div class="episode-section">
          ...
          <!-- [MODIFIED] EPISODE DISPLAY WITH RANGES AND MOVIE-STYLE BUTTONS -->
          {% if movie.episodes %}
            {% for ep in movie.episodes | sort(attribute='episode_number') | sort(attribute='season') %}
              <div class="episode-item">
                <div class="episode-info">
                  ...
                  {% if ep.title %}<p style="font-size: 0.8rem; color: var(--text-dark); margin-top: 5px; text-align: right;">{{ ep.title }}</p>{% endif %}
                </div>
                <div class="episode-buttons">
                  {% if ep.stream_link %}
                    <a href="{{ ep.stream_link }}" target="_blank" class="episode-button stream-btn"><i class="fas fa-play"></i> Stream</a>
                  {% endif %}
                  {% if ep.download_link %}
                    <a href="{{ ep.download_link }}" target="_blank" class="episode-button download-btn"><i class="fas fa-download"></i> Download</a>
                  {% endif %}
                </div>
                {% if ep.links %}
                <div class="episode-buttons" style="margin-top: 10px;">
                    {% for link in ep.links %}
                      <a href="{{ link.url }}" target="_blank" class="episode-button custom-btn"><i class="fas fa-link"></i> {{ link.text }}</a>
                    {% endfor %}
                </div>
                {% endif %}
              </div>
...
<script>
    ...
    // [MODIFIED] AddEpisodeField for ranges and stream/download links
    function addEpisodeField() {
        const c = document.getElementById('episodes_container');
        const d = document.createElement('div');
        d.className = 'dynamic-item';
        d.innerHTML = `<button type="button" onclick="this.parentElement.remove()" class="btn btn-danger">Remove Episode</button>
                       <div class="form-group"><label>Season Number:</label><input type="number" name="episode_season[]" value="1" required /></div>
                       <div class="form-group"><label>Episode Number (e.g., 1 or 1-9):</label><input type="text" name="episode_number[]" required /></div>
                       <div class="form-group"><label>Episode Title (Optional):</label><input type="text" name="episode_title[]" /></div>
                       <hr>
                       <p><b>Standard Links:</b></p>
                       <div class="form-group"><label>Streaming Link (Embed URL):</label><input type="url" name="episode_stream_link[]" /></div>
                       <div class="form-group"><label>Download Link:</label><input type="url" name="episode_download_link[]" /></div>
                       <hr>
                       <p><b>Custom Links (Optional):</b></p>
                       <div class="form-group"><label>Links (One per line: Button Text | URL):</label><textarea name="episode_links[]" rows="3" placeholder="e.g., Watch G-Drive | https://..."></textarea></div>`;
        c.appendChild(d);
    }
  ...
  <form method="post">
    ...
    <div id="episode_fields" style="display: none;">
      ...
      <fieldset><legend>Individual Episodes</legend>
        ...
        <!-- [MODIFIED] EPISODE LOOP FOR EDITING WITH RANGES AND NEW LINKS -->
        {% if movie.type == 'series' and movie.episodes %}{% for ep in movie.episodes | sort(attribute='episode_number') | sort(attribute='season') %}<div class="dynamic-item">
            ...
            <div class="form-group"><label>Episode Number (e.g., 1 or 1-9):</label><input type="text" name="episode_number[]" value="{{ ep.episode_number }}" required /></div>
            <div class="form-group"><label>Episode Title (Optional):</label><input type="text" name="episode_title[]" value="{{ ep.title or '' }}" /></div>
            <hr><p><b>Standard Links:</b></p>
            <div class="form-group"><label>Streaming Link (Embed URL):</label><input type="url" name="episode_stream_link[]" value="{{ ep.stream_link or '' }}" /></div>
            <div class="form-group"><label>Download Link:</label><input type="url" name="episode_download_link[]" value="{{ ep.download_link or '' }}" /></div>
            <hr><p><b>Custom Links (Optional):</b></p>
            <div class="form-group">
                <label>Links (One per line: Button Text | URL):</label>
                <textarea name="episode_links[]" rows="3" placeholder="e.g., Watch G-Drive | https://...">{% for link in ep.links %}{{ link.text }} | {{ link.url }}{% if not loop.last %}
...
def admin():
    if request.method == "POST":
        ...
        if movie_data['type'] == "movie":
        ...
        else: # [MODIFIED] Series logic for ranges and new links
            ...
            for s, e, t, stream, dl, links_text in zip(seasons, ep_nums, ep_titles, ep_stream_links, ep_download_links, ep_links_texts):
                if s.strip() and e.strip():
                    try:
                        ...
                        episode = {
                            "season": int(s), 
                            "episode_number": e.strip(), # Save as string for ranges
                            "title": t.strip(), 
                            "stream_link": stream.strip() or None,
                            "download_link": dl.strip() or None,
                            "links": custom_links,
                        }
                        movie_data['episodes'].append(episode)
                    except (ValueError, TypeError): 
                        print(f"WARN: Invalid episode data for S{s}E{e}.")
    ...
    if request.method == "POST":
        ...
        if content_type == "movie":
            ...
            for s, e, t, stream, dl, links_text in zip(seasons, ep_nums, ep_titles, ep_stream_links, ep_download_links, ep_links_texts):
                if s.strip() and e.strip():
                    custom_links = []
                    # Use splitlines() to handle different line endings from textarea
                    for line in links_text.strip().splitlines():
                        line = line.strip()
                        if '|' in line:
                            parts = line.split('|', 1)
                    ...
                    update_data["episodes"].append({
                        "season": int(s), 
                        "episode_number": e.strip(), # Save as string
                        "title": t.strip(),
                        "stream_link": stream.strip() or None,
                        "download_link": dl.strip() or None,
                        "links": custom_links
...
def telegram_webhook():
    ...
    if 'channel_post' in data:
        ...
        if parsed_info['type'] == 'movie':
            ...
            new_episode = {"season": parsed_info['season'], "episode_number": str(parsed_info['episode']), "message_id": post['message_id']}
            ...
            if len(parts) > 1:
                try:
                    ...
                    if len(payload_parts) == 3 and payload_parts[1].startswith('S'):
                        ...
                        s_num, e_num = int(payload_parts[1]), payload_parts[2]
                        ...
                        if episode: 
                            ...
                            file_info_text = f"S{s_num:02d}E{e_num}"
