...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .main-nav { position: fixed; top: 0; left: 0; width: 100%; padding: 10px 50px; display: flex; justify-content: space-between; align-items: center; z-index: 100; transition: background-color 0.3s ease; background: linear-gradient(to bottom, rgba(0,0,0,0.8) 10%, rgba(0,0,0,0)); }
  ...
  .hero-section { height: 85vh; position: relative; color: white; overflow: hidden; margin-top: var(--nav-height); }
...
<header class="main-nav">
    ...
    <div class="nav-links">
        ...
        <a href="{{ url_for('movies_by_category', cat_name='Hindi') }}">Hindi</a>
        <a href="{{ url_for('movies_by_category', cat_name='Bengali') }}">Bengali</a>
        <a href="{{ url_for('movies_by_category', cat_name='English') }}">Hollywood</a>
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .quality-group { margin-bottom: 15px; }
  .quality-tag { font-size: 1.1rem; font-weight: 700; color: var(--text-light); margin-bottom: 10px; }
  ...
  .custom-btn i { font-size: 1rem; }
  .custom-btn:hover { background-color: #b00710; }

  .episode-section { margin-top: 10px; }
  .episode-button { display: inline-block; padding: 12px 25px; background-color: #444; color: white; text-decoration: none; border-radius: 4px; font-weight: 700; transition: background-color 0.3s ease; margin-right: 10px; margin-bottom: 10px; text-align: center; vertical-align: middle; }
  ...
  .action-btn { display: block; width: 100%; max-width: 320px; margin: 0 auto 10px auto; }
...
<div class="detail-hero" style="min-height: auto; padding-bottom: 60px;">
  ...
  <div class="detail-content-wrapper"><img class="detail-poster" src="{{ movie.poster or 'https://via.placeholder.com/400x600.png?text=No+Image' }}" alt="{{ movie.title }}">
    <div class="detail-info">
      ...
      <div style="margin: 20px 0;"><a href="{{ url_for('contact', report_id=movie._id, title=movie.title) }}" class="custom-btn" style="background-color:#5a5a5a; text-align:center; display:block;"><i class="fas fa-flag"></i> Report a Problem</a></div>
      
      {% if movie.is_coming_soon %}<h3 class="section-title">Coming Soon</h3>
      
      {% elif movie.type == 'movie' %}
        <!-- [MODIFIED] New flexible download button system -->
        <div class="download-options">
          <h3 class="section-title">Download or Watch Online</h3>
          {% if movie.download_options %}
            {% for option in movie.download_options %}
              <div class="quality-group">
                <h4 class="quality-tag">{{ option.quality_tag }}</h4>
                <div class="button-group">
                  {% for button in option.buttons %}
                    <a href="{{ button.url }}" class="custom-btn" target="_blank" rel="noopener">
                      {% if 'down' in button.text|lower %}<i class="fas fa-download"></i>
                      {% elif 'file' in button.text|lower or 'tele' in button.text|lower %}<i class="far fa-folder"></i>
                      {% elif 'watch' in button.text|lower or 'play' in button.text|lower or 'stream' in button.text|lower %}<i class="fas fa-play"></i>
                      {% endif %}
                      {{ button.text }}
                    </a>
                  {% endfor %}
            ...
            {% endfor %}
          {% else %}
             <p>No download or watch links available yet.</p>
          {% endif %}
        </div>
...
{% if related_movies %}<div class="related-section-container"><h3 class="section-title" style="margin-left: 50px; color: white;">More Movie Recommendations</h3><div class="related-grid">{% for m in related_movies %}{{ render_movie_card(m) }}{% endfor %}</div></div>{% endif %}
...
<head>
    ...
    <style>
        ...
        .remove-btn { position: absolute; top: 10px; right: 10px; padding: 4px 8px; font-size: 0.8rem; }
...
<div class="admin-container">
    ...
    <form method="post" action="{{ url_for('admin') }}" id="content_form">
        <!-- [MODIFIED] Hidden textarea for new download options -->
        <input type="hidden" name="download_options_json" id="download_options_json">

        <fieldset>
            <legend>Manual Method / Details</legend>
        ...
        <!-- [MODIFIED] New Dynamic Download Options for Movies -->
        <div id="movie_fields">
            <fieldset><legend>Movie Download & Watch Options</legend>
                <div id="download_options_container"></div>
                <button type="button" onclick="addQualityGroup()" class="btn btn-secondary"><i class="fas fa-plus"></i> Add Quality Group</button>
                <p style="font-size: 0.8rem; color: var(--text-medium); margin-top: 10px;">For 'Get File' buttons, use the full Telegram URL: https://t.me/{{bot_username}}?start=MOVIE_ID_QUALITY</p>
            </fieldset>
        </div>
...
<script>
    ...
    function addQualityGroup(group = {}) {
        ...
        groupEl.innerHTML = \`
            ...
            <div class="form-group">
                ...
                <input type="text" class="quality-tag-input" value="\${group.quality_tag || ''}" required>
            </div>
        ...
        if (group.buttons) {
            group.buttons.forEach(button => addButton(buttonsContainer, button));
        }
    }

    function addButton(container, button = {}) {
        const buttonEl = document.createElement('div');
        buttonEl.className = 'dynamic-item button-item';
        buttonEl.innerHTML = \`
            <button type="button" class="btn btn-danger remove-btn" style="padding:2px 6px; font-size:0.7rem;" onclick="this.parentElement.remove()">X</button>
            <div style="display:flex; gap:10px;">
                <div class="form-group" style="flex:1;"><label>Button Text</label><input type="text" class="button-text-input" value="\${button.text || ''}" required></div>
                <div class="form-group" style="flex:2;"><label>Button URL</label><input type="url" class="button-url-input" value="\${button.url || ''}" required></div>
            </div>
        \`;
        container.appendChild(buttonEl);
    }
    
    // [NEW] Serialize download options before form submission
    document.getElementById('content_form').addEventListener('submit', function(e) {
        const downloadOptions = [];
        document.querySelectorAll('.quality-group-item').forEach(groupEl => {
            const quality_tag = groupEl.querySelector('.quality-tag-input').value;
            const buttons = [];
            groupEl.querySelectorAll('.button-item').forEach(buttonEl => {
                const text = buttonEl.querySelector('.button-text-input').value;
                const url = buttonEl.querySelector('.button-url-input').value;
                if (text && url) {
                    buttons.push({ text, url });
                }
            });
            if (quality_tag && buttons.length > 0) {
                downloadOptions.push({ quality_tag, buttons });
            }
        });
        document.getElementById('download_options_json').value = JSON.stringify(downloadOptions);
    });
    ...
    async function fetchFromTmdb() {
        ...
        try {
            ...
            document.querySelector('#content_form [name="title"]').value = data.title || '';
            document.querySelector('#content_form [name="overview"]').value = data.overview || '';
            document.querySelector('#content_form [name="poster"]').value = data.poster || '';
            document.querySelector('#content_form [name="genres"]').value = data.genres ? data.genres.join(', ') : '';
            
            const contentTypeSelect = document.querySelector('#content_form [name="content_type"]');
            contentTypeSelect.value = data.type === 'series' ? 'series' : 'movie';
            toggleFields();
            
            if (data.trailer_link) {
                document.querySelector('#content_form [name="trailer_link"]').value = data.trailer_link;
            }
      ...
      <fieldset><legend>Season Packs</legend>
        <div id="season_packs_container">
          ...
          <div class="dynamic-item">
            <button type="button" onclick="this.parentElement.remove()" class="btn btn-danger remove-btn">Remove</button>
...
<script>
    ...
    // [NEW] Load existing download options on page load
    document.addEventListener('DOMContentLoaded', () => {
        toggleFields();
        const existingOptions = {{ movie.download_options|tojson|safe if movie.download_options else [] }};
        if (existingOptions) {
            existingOptions.forEach(group => addQualityGroup(group));
        }
    });
</script>
</body></html>
...
def get_tmdb_details_from_title(title, content_type, year=None):
    ...
    try:
        ...
        return get_tmdb_details_from_api(tmdb_id, content_type)
    except Exception as e:
...
def home():
    ...
    limit = 12
    # [MODIFIED] Homepage context with new categories
    context = {
...
def watch_movie(movie_id):
    try:
        movie = movies.find_one({"_id": ObjectId(movie_id)})
        # [MODIFIED] Check for watch link in the new download_options as well
        watch_link = movie.get("watch_link") 
        if not watch_link and movie.get('download_options'):
             for group in movie['download_options']:
                for button in group['buttons']:
                    if 'watch' in button.get('text','').lower() or 'stream' in button.get('text','').lower():
                        watch_link = button.get('url')
                        break
                if watch_link:
                    break
        
        if not watch_link: return "Content not found.", 404
        return render_template_string(watch_html, watch_link=watch_link, title=movie["title"])
...
def admin():
    if request.method == "POST":
        # [MODIFIED] Admin POST logic to handle new download options
        movie_data = {
            ...
            "episodes": [], "season_packs": [], "languages": [],
            "download_options": json.loads(request.form.get("download_options_json", "[]"))
        }
        
        if not movie_data.get('tmdb_id'):
            tmdb_details = get_tmdb_details_from_title(movie_data['title'], movie_data['type'])
            if tmdb_details:
                tmdb_details.update(movie_data)
                movie_data = tmdb_details
...
def fetch_tmdb_data():
    ...
    try:
        ...
        if len(path_parts) < 2 or path_parts[0] not in ['movie', 'tv']:
            raise ValueError("Invalid TMDB URL format.")
        content_type = "series" if path_parts[0] == 'tv' else "movie"
        tmdb_id = path_parts[1].split('-')[0]
        if not tmdb_id.isdigit():
            raise ValueError("Could not extract a valid ID from the URL.")
        details = get_tmdb_details_from_api(tmdb_id, content_type)
        if details:
            return jsonify(details)
        else:
            return jsonify({"error": "Could not find details for this ID."}), 404
...
def edit_movie(movie_id):
    ...
    if request.method == "POST":
        ...
        if content_type == "movie":
            update_data["download_options"] = json.loads(request.form.get("download_options_json", "[]"))
            movies.update_one({"_id": obj_id}, {"$set": update_data, "$unset": {"episodes": "", "season_packs": "", "links": "", "files": "", "watch_link": ""}})
        else: # series
            ...
            movies.update_one({"_id": obj_id}, {"$set": update_data, "$unset": {"download_options": "", "links": "", "files": "", "watch_link": ""}})
...
def telegram_webhook():
    ...
    if 'channel_post' in data:
        ...
        def get_or_create_content_entry(tmdb_details, parsed_details):
            if tmdb_details and tmdb_details.get("tmdb_id"):
                ...
                if not existing_entry:
                    base_doc = {**tmdb_details, "type": "movie" if parsed_details['type'] == 'movie' else "series", "languages": [], "episodes": [], "season_packs": [], "categories": [], "download_options": [], "is_coming_soon": False}
                ...
                if not existing_entry:
                    shell_doc = {"title": parsed_details['title'], "type": "movie" if parsed_details['type'] == 'movie' else "series", "poster": PLACEHOLDER_POSTER, "overview": "Details will be updated soon.", "release_date": None, "genres": [], "vote_average": 0, "trailer_link": None, "tmdb_id": None, "languages": [], "episodes": [], "season_packs": [], "categories": [], "download_options": [], "is_coming_soon": False}
        ...
        # This webhook logic now only supports Series files. Movie files must be added via the new Admin Panel.
        if parsed_info['type'] == 'series_pack':
            ...
            if len(parts) > 1:
                try:
                    ...
                    if len(payload_parts) == 3 and payload_parts[1].startswith('S'): # Season Pack
                        season_num, quality = int(payload_parts[1][1:]), payload_parts[2]
                        pack = next((p for p in content.get('season_packs', []) if p.get('season') == season_num and p.get('quality') == quality), None)
                        if pack: message_to_copy_id, file_info_text = pack.get('message_id'), f"Complete Season {season_num} ({quality})"
                    elif content.get('type') == 'series' and len(payload_parts) == 3: # Episode
                    ...
                    if message_to_copy_id:
                        ...
                        else: requests.get(f"{TELEGRAM_API_URL}/sendMessage", params={'chat_id': chat_id, 'text': "Error sending file. It might have been deleted."})
...
def send_manual_notification(movie_id):
    try:
        ...
        movie_obj = movies.find_one({"_id": obj_id})
        if movie_obj: send_notification_to_channel(movie_obj)
    except Exception as e: print(f"ERROR in send_manual_notification for ID {movie_id}: {e}")
    return redirect(url_for('admin'))
