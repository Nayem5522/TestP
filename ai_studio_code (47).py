...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .download-section, .episode-section { margin-top: 10px; }
    ...
    <div class="detail-info">
      ...
      {% elif movie.type == 'movie' %}
        <div class="download-section">
          <!-- [MODIFIED] Custom Download Links -->
          {% if movie.links %}<h3 class="section-title">Download Links</h3>{% for link_item in movie.links %}<div><a class="download-button" href="{{ link_item.url }}" target="_blank" rel="noopener"><i class="fas fa-download"></i> {{ link_item.text }}</a><button class="copy-button" onclick="copyToClipboard('{{ link_item.url }}')"><i class="fas fa-copy"></i></button></div>{% endfor %}{% endif %}
    ...
    <form method="post" action="{{ url_for('admin') }}">
        ...
        <div id="movie_fields">
            <fieldset><legend>Movie Links</legend>
                ...
                <!-- [MODIFIED] Dynamic Download Links -->
                <p><b>Direct Download Links</b></p>
                <div id="download_links_container"></div>
                <button type="button" onclick="addDownloadLinkField()" class="btn btn-secondary"><i class="fas fa-plus"></i> Add Download Link</button>
                <hr>
...
<div class="admin-container">
  ...
  <form method="post">
    ...
    <div id="movie_fields">
        <fieldset><legend>Movie Links</legend>
            <div class="form-group"><label>Watch Link:</label><input type="url" name="watch_link" value="{{ movie.watch_link or '' }}" /></div><hr>
            
            <!-- [MODIFIED] Dynamic Download Links for Editing -->
            <p><b>Direct Download Links</b></p>
            <div id="download_links_container">
                {% if movie.type == 'movie' and movie.links %}{% for link in movie.links %}
                <div class="dynamic-item">
                    <button type="button" onclick="this.parentElement.remove()" class="btn btn-danger"><i class="fas fa-times"></i></button>
                    <div class="form-group"><label>Button Text:</label><input type="text" name="link_text[]" value="{{ link.text }}" required /></div>
                    <div class="form-group"><label>URL:</label><input type="url" name="link_url[]" value="{{ link.url }}" required /></div>
                </div>
                {% endfor %}{% endif %}
            </div>
            <button type="button" onclick="addDownloadLinkField()" class="btn btn-secondary"><i class="fas fa-plus"></i> Add Download Link</button>
...
def admin():
    if request.method == "POST":
        ...
        if movie_data['type'] == "movie":
            ...
            # [MODIFIED] Process custom download links
            ...
            for text, url in zip(link_texts, link_urls):
                if text.strip() and url.strip():
                    movie_data['links'].append({"text": text.strip(), "url": url.strip()})
...
def edit_movie(movie_id):
    ...
    if request.method == "POST":
        ...
        if content_type == "movie":
            ...
            # [MODIFIED] Process custom download links on edit
            link_texts = request.form.getlist('link_text[]')
            link_urls = request.form.getlist('link_url[]')
            update_data['links'] = []
            for text, url in zip(link_texts, link_urls):
                if text.strip() and url.strip():
                    update_data['links'].append({"text": text.strip(), "url": url.strip()})
